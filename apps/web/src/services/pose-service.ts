...
class PoseService {
  ...
  private readonly previousCentroids = new Map<string, { x: number; y: number }>()
  ...
  buildDemoFrame(time: number): PoseFrame {
    ...
    const keypoints: PoseKeypoint[] = [
      { index: 0, name: "nose", x: cx, y: cy - 0.17 + breathe, confidence: 0.98 },
      { index: 1, name: "left_eye", x: cx - 0.018, y: cy - 0.183 + breathe, confidence: 0.96 },
      { index: 2, name: "right_eye", x: cx + 0.018, y: cy - 0.183 + breathe, confidence: 0.96 },
      { index: 3, name: "left_ear", x: cx - 0.036, y: cy - 0.175 + breathe, confidence: 0.89 },
      { index: 4, name: "right_ear", x: cx + 0.036, y: cy - 0.175 + breathe, confidence: 0.89 },
      { index: 5, name: "left_shoulder", x: cx - 0.065, y: cy - 0.105 + breathe, confidence: 0.95 },
      { index: 6, name: "right_shoulder", x: cx + 0.065, y: cy - 0.105 + breathe, confidence: 0.95 },
      { index: 7, name: "left_elbow", x: cx - 0.088, y: cy - 0.02 + breathe, confidence: 0.92 },
      { index: 8, name: "right_elbow", x: cx + 0.088, y: cy - 0.02 + breathe, confidence: 0.92 },
      { index: 9, name: "left_wrist", x: cx - 0.078, y: cy + 0.06 + breathe, confidence: 0.89 },
      { index: 10, name: "right_wrist", x: cx + 0.078, y: cy + 0.06 + breathe, confidence: 0.89 },
      { index: 11, name: "left_hip", x: cx - 0.04, y: cy + 0.1 + breathe, confidence: 0.94 },
      { index: 12, name: "right_hip", x: cx + 0.04, y: cy + 0.1 + breathe, confidence: 0.94 },
      { index: 13, name: "left_knee", x: cx - 0.045, y: cy + 0.235 + stride * 0.25, confidence: 0.91 },
      { index: 14, name: "right_knee", x: cx + 0.045, y: cy + 0.235 - stride * 0.25, confidence: 0.91 },
      { index: 15, name: "left_ankle", x: cx - 0.05, y: cy + 0.37 + stride, confidence: 0.88 },
      { index: 16, name: "right_ankle", x: cx + 0.05, y: cy + 0.37 - stride, confidence: 0.88 },
    ]
  ...
  private normalizePayload(payload: ExtractedPosePayload): PoseFrame | null {
    ...
    const normalizedPersons = parsedPersons.map((person) => {
      const keypoints = this.normalizeKeypoints(person.keypoints, bounds)
  ...
  private extractPosePayload(message: WebSocketMessage): ExtractedPosePayload | null {
  ...
  private normalizeKeypoints(
    ...
    const slots = Array.from({ length: COCO_KEYPOINT_NAMES.length }, (_, index) => ({
      index,
  ...
  private enrichPerson(input: {
    ...
    const movementDelta = previous ? distance(centroid, previous) : 0
  ...
  private calculateCentroid(keypoints: PoseKeypoint[]): { x: number; y: number } {
  ...
  private buildStats(
    ...
    const signalStrengthFromMetadata = toNumber(context.metadata.signal_strength)
    ...
    const trackingPoints = persons.reduce(
      (count, person) =>
        count + person.keypoints.filter((keypoint) => keypoint.confidence > 0.3).length,
      0
    )
    ...
    const movementLabel =
      (primary?.movementDelta ?? 0) > 0.03
        ? "High"
        : (primary?.movementDelta ?? 0) > 0.009
          ? "Active"
          : "Minimal"

    const postureLabel = primary ? "Standing" : "Unknown"

    return {
      signalStrength,
      trackingPoints,
      latency: Math.max(Math.round(context.processingTimeMs || 0), 8),
      roomDuration: Math.max(1, Math.floor((Date.now() - this.startedAt) / 60000)),
